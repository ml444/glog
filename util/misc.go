...
func ConstructFieldIndexMap(record interface{}) map[string]int {
	dataT := reflect.TypeOf(record)
	m := map[string]int{}
...
func parseFormatTemp(src string) string {
	...
	regexpReplaceFunc := func(s string) string {
		...
		if !ok {
			panic(fmt.Sprintf("%s in config.Text.PatternStyle,But it isn't in the field of record", s))
		}
